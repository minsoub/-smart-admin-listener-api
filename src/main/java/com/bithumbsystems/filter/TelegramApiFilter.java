...
@Slf4j
@Component
public class TelegramApiFilter extends AbstractGatewayFilterFactory<Config> {

  @Value("${sites.telegram-url}")
  private String telegramUrl;

  public TelegramApiFilter() {
    super(Config.class);
  }
  ...
  public GatewayFilter apply(final Config config) {
    return (exchange, chain) -> {
      ...
      AtomicReference<String> goUrl = new AtomicReference<>(telegramUrl);
      log.debug("goUrl:{}", goUrl.get());

      String pathOrigin = exchange.getRequest().getURI().getPath();
      String path = pathOrigin.replaceAll("/telegram", "");
